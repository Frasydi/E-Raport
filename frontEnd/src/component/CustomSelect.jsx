...
const CustomSelect = ({
    ...
    displayKey = "label",
    valueKey = "value",
    emptyMessage = "Data belum ada",
    allowEmpty = false,         // <-- Tambahan
    emptyLabel = "Kosong",      // <-- Tambahan
}) => {
        ...
        <div ref={dropdownRef} className="relative">
            ...
            >
                <span className={!isValueValid() ? "text-xs" : ""}>
                    {isValueValid() ? getDisplayText(getSelectedOption()) : placeholder}
                </span>
                <svg
                    ...
                    {options.length === 0 ? (
                        ...
                        options.map((option, index) => (
                            <li
                                ...
                                className={`px-4 py-2 text-sm hover:bg-blue-100 cursor-pointer transition ${
                                    value === getValue(option) ? "bg-blue-50 text-blue-400 font-medium" : ""
                                }`}
                            >
